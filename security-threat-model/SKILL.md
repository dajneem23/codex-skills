...
## Competition-Derived Priors (EVM Repos)

When the scoped system is an EVM protocol, bias the initial threat hypothesis set toward findings repeatedly observed in `Past-Audit-Competitions`:

- Authorization binding failures (inconsistent modifiers, weak caller identity checks, and `tx.origin`-style anti-patterns).
- Accounting/invariant drift (asset-liability mismatch, fee retention desync, and transition-state corruption).
- Oracle integrity faults (staleness/confidence omissions, manipulable spot dependencies, and unsafe fallback paths).
- Replay/front-run abuse in signed execution and nonce management.
- DoS/griefing through unbounded loops, edge-state sequencing, or incomplete pause semantics.

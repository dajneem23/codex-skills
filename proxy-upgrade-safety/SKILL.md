...
- Initialization access is a recurring failure mode: permissionless initialization/constructor-equivalent flows enabled ownership capture.
- Upgrade and migration paths frequently broke state assumptions, creating latent accounting or authorization inconsistencies after transition.
- Privileged upgrade actions need stricter operational controls (multisig/timelock/change windows) to reduce takeover blast radius.
- Safety checks must cover post-upgrade behavior, not only authorization to upgrade.

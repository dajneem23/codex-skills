...
- Claim/distribution loops are recurring failure points: one bad recipient or oversized set can stall protocol-critical operations.
- Front-runnable edge-state transitions repeatedly caused griefing, especially around epoch/process rollback and partial-progress invalidation.
- Pause controls are frequently incomplete: withdrawals/claims often remained callable in “paused” states, enabling loss-amplifying behavior.
- Gas griefing remains practical in relayed/multi-call flows when failure paths consume state without robust retry/idempotency design.

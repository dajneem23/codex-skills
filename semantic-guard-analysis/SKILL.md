...
- Inconsistent pause semantics are repeatedly exploitable: one function enforces pause while a sibling path bypasses it.
- Authorization checks drift over time; mixed guard styles (`onlyOwner`, inline `require`, `tx.origin`) created bypass opportunities.
- Edge-case admin paths often omit the “normal” checks used elsewhere, producing high-impact semantic outliers.
- Privileged state mutation without matching guard consistency remains one of the most reliable logic-bug signals.

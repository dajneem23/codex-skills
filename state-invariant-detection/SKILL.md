...
- Accounting discrepancy findings are repeatedly critical: one-sided updates to liabilities/assets caused insolvency and frozen exits.
- Delegation/governance state drift after slashing/unlock/epoch transitions repeatedly broke entitlement invariants.
- Interest and present-value paths commonly leak invariant integrity through rounding/truncation under long-running accrual.
- Pause/recovery paths must preserve accounting equations; emergency code often introduced irreversible state desynchronization.

...
When the target repo is an EVM protocol or smart-contract system, prioritize ownership analysis on files that tend to drive recurring criticals:
- Access control surfaces (modifiers, role managers, upgrade/auth paths) — repeated privileged bypasses stem from drift here.
- Accounting and invariant-critical modules (vaults, lending, fee/interest logic) — insolvency issues cluster in these areas.
- Oracle/price integration points — stale/manipulable feeds often localized to specific adapter files.
- Signature/permit/nonce handlers — replay/front-run bugs frequently live in dedicated verification modules.
- Pause/administrative control flows — operational safety paths often diverge in enforcement and deserve dedicated ownership clarity.

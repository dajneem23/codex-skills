...
When the repo is a smart-contract/EVM codebase, bias reviews toward these recurring high-impact classes observed across competition reports:
- Authorization binding gaps (mixed modifiers, weak caller validation, `tx.origin` anti-patterns).
- Accounting/invariant drift (asymmetric updates in fee/interest/epoch transitions) leading to insolvency.
- Oracle/price validation omissions (stale/confidence checks, manipulable spot dependencies, unsafe fallbacks).
- Replay/front-run issues in signature/nonce handling (missing domain binding, loose nonce lifecycle, absent deadlines).
- DoS/griefing via unbounded loops, incomplete pause semantics, or edge-state sequencing bugs.

...
- Reentrancy frequently appears in composition paths (flash loans, callbacks, multi-function interactions), not just in a single obvious withdraw function.
- Cross-function state sharing remains a recurring root cause when one path is guarded and another mutates the same state without equivalent protection.
- Callback-aware token integrations continue to create hidden reentry windows, especially where accounting updates trail transfer/call boundaries.
- Read-only dependency surfaces can still be exploited when stale values are consumed by downstream protocols during callback windows.

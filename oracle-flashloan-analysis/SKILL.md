...
- Oracle confidence/freshness validation is repeatedly incomplete; stale-but-nonzero prices and weak confidence gates caused major misvaluation.
- Single-source or spot-derived pricing remains a practical flash-loan attack surface for borrow, liquidation, and mint paths.
- Hardcoded/constant confidence assumptions are fragile and repeatedly observed as root cause in critical findings.
- Price-path safety must include fallback behavior; unsafe fallback activation rules frequently converted data issues into exploitable states.

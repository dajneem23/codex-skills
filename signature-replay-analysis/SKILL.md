...
- Missing chain/domain binding remains a common replay root cause; signatures must bind `chainId` and `verifyingContract`.
- Nonce lifecycle flaws are repeatedly exploitable via front-run or out-of-order consumption in relayed execution flows.
- Deadline omissions turn benign signatures into durable attack assets that can be executed under changed market/state conditions.
- Signature checks must be strict on signer recovery and message scope to prevent cross-context reuse.

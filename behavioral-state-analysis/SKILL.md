...
- Prioritize accounting drift checks first: insolvency often came from asymmetric updates across deposit/withdraw/fee paths.
- Treat authorization binding as high-priority: repeated findings show ownership checks bypassed via weak caller validation (including `tx.origin` misuse).
- Elevate oracle quality checks: stale feeds, weak confidence handling, and constant-based confidence thresholds repeatedly produced exploitable pricing.
- Model pause/sequence abuse: many incidents were griefing or privilege outcomes where one flow enforced checks and another flow bypassed them.

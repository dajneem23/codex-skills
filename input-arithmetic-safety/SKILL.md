...
- Decimal normalization errors are frequent and high impact; mismatched asset decimals repeatedly produced broken collateral and borrow valuations.
- Rounding direction and truncation issues repeatedly leaked value over time in interest, conversion, and present-value paths.
- Unsafe constants in confidence/precision checks caused false safety assumptions and oracle-driven mispricing.
- Input bounds on privileged setters remain a common weakness; unchecked parameter ranges can silently rewrite protocol economics.

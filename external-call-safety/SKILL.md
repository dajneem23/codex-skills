...
- Token-behavior assumptions are a recurring break point: decimal mismatch, fee-on-transfer behavior, and blacklist mechanics repeatedly broke accounting.
- External call failures in batch logic often escalated to protocol-level DoS when one revert blocked all progress.
- Callback-capable token paths continue to expose reentrancy windows when state mutation ordering is incomplete.
- Low-level call hygiene still matters: unchecked success paths and permissive fallback logic repeatedly surfaced as exploitable inconsistencies.
